function ripsercallback(src, eventData, pointCloud, dx)
%{
   This function was adapted from the following "Click3dpoint", 
    found online on mathworks file exchange:
...
%{
The new code which plots the point cloud, highlighting the basepoint
and coloring the point cloud based on how close points are to the
selected basepoint. Then calls computeripserfiltration with X the 
point cloud, pointcloudindex the selected basepoint's index, and 
dx the distance matrix, to compute the barcodes
%}

ecc = dx(:,pointCloudIndex);
scatter3(pointCloud(1,:), pointCloud(2,:), pointCloud(3,:), 25, ecc, 'filled');
hold on;
scatter3(pointCloud(1,pointCloudIndex), pointCloud(2,pointCloudIndex), pointCloud(3,pointCloudIndex), 30, 'r', 'filled');
X = pointCloud;
computeripserfiltration(X,pointCloudIndex, dx);
